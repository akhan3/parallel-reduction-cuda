...
//     unsigned int num_threads = 512;
    ...
    for(unsigned int i = N; i > 1; i = i/2){
        ...
        dim3  grid( 1, 1, 1);
        dim3  threads(i/2, 1, 1);
        step = N/i;

        printf("threads: %d, step: %d\n", threads.x, step);

        // execute the kernel
        testKernel<<< grid, threads, mem_size >>>(d_idata, d_odata, step);

        // check if kernel execution generated and error
        cutilCheckMsg("Kernel execution failed");
    }
    cudaMemcpy(&temp_float, d_idata, sizeof(float), cudaMemcpyDeviceToHost);
    printf("The maximum is found to be %f\n", temp_float);
